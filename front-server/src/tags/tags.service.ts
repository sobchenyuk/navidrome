...
export class TagsService {
  private readonly musicPath = process.env.MUSIC_PATH || '/Volumes/workFlow/startup/navidrome/music'; // Local development path
  ...
  async updateTags(filePath: string, tagInput: TagInput): Promise<boolean> {
    try {
      ...
      console.log(`🏷️ Updating tags for ${filePath}:`, tagInput);
      ...
      if (ext === '.mp3') {
        ...
        if (success) {
          console.log(`✅ Successfully updated MP3 tags for ${filePath}`);
          return true;
        } else {
          console.error(`❌ Failed to update MP3 tags for ${filePath}`);
          return false;
        }
      } else {
